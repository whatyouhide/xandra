defmodule Xandra.Cluster.ControlConnection2Test do
  ...
  test "fails to start if it can't connect to the contact point node",
    ...
    assert {:error, _} = start_supervised({ControlConnection2, options})
  ...
  test "dies if the connected node closes its socket",
    ...
    assert {:ok, ctrl_conn} = start_supervised({ControlConnection2, start_options})
  ...
  defp start_control_connection!(start_options, overrides \\ []) do
    ...
    TestHelper.start_link_supervised!({ControlConnection2, options})
  end
